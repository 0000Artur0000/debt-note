...
class ObligationServiceTest {
    ...
    @Test
    void dublikatDaetPreduprezhdenie() {
        when(repo.existsByNazvanieIgnoreCaseAndStatus("test", Status.ACTIVE)).thenReturn(true);
        var z = new CreateObligationRequest("test", new BigDecimal("100"), "RUB",
            Category.SUBSCRIPTION, null, LocalDate.of(2026, 8, 1));
        assertThat(servis.sozdat(z).preduprezhdenie()).isNotNull();
    }
    @ParameterizedTest
    @CsvSource({"MONTHLY,2025-01-31,2025-02-28","MONTHLY,2024-01-31,2024-02-29",
        "QUARTERLY,2025-01-15,2025-04-15","YEARLY,2024-02-29,2025-02-28"})
    void sdvigDaty(Recurrence r, LocalDate ot, LocalDate ozh) { assertThat(r.sleduyushchaya(ot)).isEqualTo(ozh); }
    @Test
    void neAktivnyNelzyaOplatit() {
